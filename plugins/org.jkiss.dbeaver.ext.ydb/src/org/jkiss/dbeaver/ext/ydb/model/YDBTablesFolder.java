...
/**
 * YDB Tables container folder.
 * This is a top-level folder that contains all table folders and root tables.
 */
public class YDBTablesFolder implements DBSFolder {
    ...
    public YDBTablesFolder(@NotNull YDBDataSource dataSource) {
    ...
    private synchronized void buildHierarchy(@NotNull DBRProgressMonitor monitor) throws DBException {
        ...
        List<? extends GenericTableBase> allTables = dataSource.getTables(monitor);
        if (allTables == null) {
            hierarchyBuilt = true;
            return;
        }
