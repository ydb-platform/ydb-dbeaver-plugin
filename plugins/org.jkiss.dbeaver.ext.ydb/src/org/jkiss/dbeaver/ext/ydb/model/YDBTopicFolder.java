...
/**
 * Virtual folder for hierarchical topic organization.
 */
public class YDBTopicFolder implements DBSFolder, YDBPermissionHolder {

    private static final Log log = Log.getLog(YDBTopicFolder.class);

    private final DBSObject owner;
    private final YDBTopicFolder parentFolder;
    private final String name;
    private final String fullPath;
    private final Map<String, YDBTopicFolder> subFolders = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
    private final List<YDBTopic> topics = new ArrayList<>();
    ...
    public YDBTopicFolder(
        @NotNull DBSObject owner,
        @Nullable YDBTopicFolder parentFolder,
        @NotNull String name
    ) {
        this.owner = owner;
    ...
    public Collection<DBSObject> getChildrenObjects(@NotNull DBRProgressMonitor monitor) throws DBException {
        ...
        children.addAll(topics);
        return children;
    }

    @Association @NotNull
    public Collection<YDBTopicFolder> getSubFolders() { return subFolders.values(); }

    @Association @NotNull
    public List<YDBTopic> getTopics() {
        topics.sort(Comparator.comparing(YDBTopic::getName, String.CASE_INSENSITIVE_ORDER));
        return topics;
    }

    public void addTopic(@NotNull YDBTopic topic) { topics.add(topic); }

    @NotNull
    public YDBTopicFolder getOrCreateSubFolder(@NotNull String folderName) {
        return subFolders.computeIfAbsent(folderName, n -> new YDBTopicFolder(owner, this, n));
    }
