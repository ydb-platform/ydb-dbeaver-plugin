...
/**
 * Virtual folder for hierarchical transfer organization.
 */
public class YDBTransferFolder implements DBSFolder, YDBPermissionHolder {

    private static final Log log = Log.getLog(YDBTransferFolder.class);

    private final DBSObject owner;
    private final YDBTransferFolder parentFolder;
    private final String name;
    private final String fullPath;
    private final Map<String, YDBTransferFolder> subFolders = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
    private final List<YDBTransfer> transfers = new ArrayList<>();
    ...
    public YDBTransferFolder(
        @NotNull DBSObject owner,
        @Nullable YDBTransferFolder parentFolder,
        @NotNull String name
    ) {
        this.owner = owner;
    ...
    public Collection<DBSObject> getChildrenObjects(@NotNull DBRProgressMonitor monitor) throws DBException {
        ...
        children.addAll(transfers);
        return children;
    }

    @Association @NotNull
    public Collection<YDBTransferFolder> getSubFolders() { return subFolders.values(); }

    @Association @NotNull
    public List<YDBTransfer> getTransfers() {
        transfers.sort(Comparator.comparing(YDBTransfer::getName, String.CASE_INSENSITIVE_ORDER));
        return transfers;
    }

    public void addTransfer(@NotNull YDBTransfer transfer) { transfers.add(transfer); }

    @NotNull
    public YDBTransferFolder getOrCreateSubFolder(@NotNull String folderName) {
        return subFolders.computeIfAbsent(folderName, n -> new YDBTransferFolder(owner, this, n));
    }
